 ...
 * \class   antex
 *
 * \details This class provides am interface to the igs ANTEX files. Users can
 *          construct instances by providing a (valid) ANTEX filename.
 *          Information is extracted via the ngpt::antenna_pcv<pcv_type> class;
 *          i.e. to extract information (pcv, offsets, etc) for a given antenna,
 *          a user must first read off (from the instance) an antenna_pcv<>
 *          instance.
 *
 * \warning The class only knows how te **read** ANTEX files, i.e. they are
...
class antex
{
    ...
    enum class PCV_TYPE : char { Absolute, Relative };
...
public:
    ...
    //     Also, since we may know where the fuck the antenna is inside the file,
    ...
    std::vector<ant_pos_pair> get_antenna_list();
